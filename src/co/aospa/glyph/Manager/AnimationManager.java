...
public final class AnimationManager {
    ...
    public static void playCharging(int batteryLevel, boolean wait) {
        submit(() -> {
            ...
            int amount = (int) (Math.floor((batteryLevel / 100.0) * (batteryArray.length - (batteryDot ? 2 : 1))) + 1);

            try {
                if (batteryDot) batteryArray[0] = Constants.getBrightness();
                for (int i = (batteryDot ? 1 : 0); i < batteryArray.length; i++) {
